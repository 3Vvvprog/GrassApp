//
//  Colors.swift
//  GrassApp
//
//  Created by Вячеслав Вовк on 20.09.2023.
//

import Foundation
import UIKit

struct Colors {
    static let mainColorGreen = UIColor(red: 0.125, green: 0.722, blue: 0.294, alpha: 1)
    static let lightGreen = UIColor(red: 166/255.0, green: 227/255.0, blue: 184/255.0, alpha: 1.000)
    static let backgroundColor = UIColor(red: 0.967, green: 0.964, blue: 0.964, alpha: 1)
    
    static let buttonGray = UIColor(red: 0.914, green: 0.929, blue: 0.938, alpha: 1)
}
